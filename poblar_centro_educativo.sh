...
# Asignando asignaturas a la Profesora Ana López (ejemplo con nuevos datos)
PROFESOR_DNI_ANA="23456733H"
ASIGNATURAS_PARA_ANA=("IAP") # Asignaturas que impartirá Ana

echo "--- Asignando asignaturas a la Profesora Ana López ($PROFESOR_DNI_ANA)... ---"

for ACRONIMO_ASIG in "${ASIGNATURAS_PARA_ANA[@]}"; do
  echo "  Asignando asignatura: $ACRONIMO_ASIG a profesor $PROFESOR_DNI_ANA"
  
  RESPONSE_ASIG_PROF_BODY_AND_CODE=$(curl -s -w "\nHTTP_STATUS_ASIG_A_PROF:%{http_code}\n" \
    --data "$ACRONIMO_ASIG" \
    -X POST "${BASE_URL}/profesores/${PROFESOR_DNI_ANA}/asignaturas?key=$ADMIN_API_KEY" \
    -H "Content-Type: application/json" \
    -b $ADMIN_COOKIE_FILE -c $ADMIN_COOKIE_FILE)
    
  echo "    Respuesta al asignar $ACRONIMO_ASIG:"
  echo -e "$RESPONSE_ASIG_PROF_BODY_AND_CODE"
done
echo ""

# Asignando asignaturas a la Profesora Ana López (ejemplo con nuevos datos)
PROFESOR_DNI_1="10293756L"
ASIGNATURAS_PARA_1=("DEW") # Asignaturas que impartirá Ana

echo "--- Asignando asignaturas a la Profesora Ana López ($PROFESOR_DNI_1)... ---"

for ACRONIMO_ASIG_1 in "${ASIGNATURAS_PARA_1[@]}"; do
  echo "  Asignando asignatura: $ACRONIMO_ASIG_1 a profesor $PROFESOR_DNI_1"
  
  RESPONSE_ASIG_PROF_BODY_AND_CODE=$(curl -s -w "\nHTTP_STATUS_ASIG_A_PROF:%{http_code}\n" \
    --data "$ACRONIMO_ASIG_1" \
    -X POST "${BASE_URL}/profesores/${PROFESOR_DNI_1}/asignaturas?key=$ADMIN_API_KEY" \
    -H "Content-Type: application/json" \
    -b $ADMIN_COOKIE_FILE -c $ADMIN_COOKIE_FILE)
    
  echo "    Respuesta al asignar $ACRONIMO_ASIG_1:"
  echo -e "$RESPONSE_ASIG_PROF_BODY_AND_CODE"
done
echo ""

# Asignando asignaturas a la Profesora Ana López (ejemplo con nuevos datos)
PROFESOR_DNI_2="65748923M"
ASIGNATURAS_PARA_2=("DEW") # Asignaturas que impartirá Ana

echo "--- Asignando asignaturas a la Profesora Ana López ($PROFESOR_DNI_2)... ---"

for ACRONIMO_ASIG_2 in "${ASIGNATURAS_PARA_2[@]}"; do
  echo "  Asignando asignatura: $ACRONIMO_ASIG_2 a profesor $PROFESOR_DNI_2"
  
  RESPONSE_ASIG_PROF_BODY_AND_CODE=$(curl -s -w "\nHTTP_STATUS_ASIG_A_PROF:%{http_code}\n" \
    --data "$ACRONIMO_ASIG_2" \
    -X POST "${BASE_URL}/profesores/${PROFESOR_DNI_2}/asignaturas?key=$ADMIN_API_KEY" \
    -H "Content-Type: application/json" \
    -b $ADMIN_COOKIE_FILE -c $ADMIN_COOKIE_FILE)
    
  echo "    Respuesta al asignar $ACRONIMO_ASIG_2:"
  echo -e "$RESPONSE_ASIG_PROF_BODY_AND_CODE"
done
echo ""
